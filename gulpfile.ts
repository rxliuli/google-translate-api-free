...
查看文档 [${name}](https://github.com/rxliuli/google-translate-api-browser)
...
async function writeReadme(dirName: string, text: string) {
  const src = path.resolve(dirName, 'README.md')
...
export async function generateReadmes() {
  ...
  pkgJsonList.forEach(([file, item]) => {
    const text = generateReadme(item)
    writeReadme(path.dirname(file), text)
  })
}
